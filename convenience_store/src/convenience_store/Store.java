...
public class Store {
	 ...
	 * 상품 판매
	 * @param sectionName 진열대 이름 (A: A진열대, B: B진열대)
	 * @param setionItemIndex 진열대의 상품 인덱스
	 * @param sellCount 판매 개수
	 */
	public void sell(String sectionName, int setionItemIndex, int sellCount) {
		if (sectionName == "A") {
			int itemStock = sectionA.items[setionItemIndex].stock;
			if (itemStock >= sellCount) {
				sectionA.items[setionItemIndex].stock -= sellCount;
				safe += sectionA.items[setionItemIndex].price;
			}
		} else if (sectionName == "B") {
			int itemStock = sectionB.items[setionItemIndex].stock;
			if (itemStock >= sellCount) {
				sectionB.items[setionItemIndex].stock -= sellCount;
				safe += sectionB.items[setionItemIndex].price;
			}
		}
	}
